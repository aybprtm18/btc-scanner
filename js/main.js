...
async function checkBalance(address, mnemonic) {
    try {
        ...
        updateStatus("Error checking balance");
    }
}
...
function displayWallet(wallet) {
    ...
    div.innerHTML = `
        ...
        <p><strong>Balance:</strong> ${wallet.balance} satoshi</p>
        <hr/>
    `;
    infoDiv.prepend(div);
}
...
function startAuto() {
    autoGenerating = true;
    updateStatus("Auto scanning started...");
    const loop = async () => {
        if (!autoGenerating) return;
        await generateWallet();
        setTimeout(loop, 1000); // Delay per wallet scan
    };
    loop();
}

function exportWallets() {
    const header = "Address,Mnemonic,Balance
";
    const csvContent = header + wallets.map(w => `${w.address},${w.mnemonic},${w.balance}`).join("\n");
